...
const buildDictTrie = function (dict,dictfreq=null) {
    var p,i, ci, totalfreq = 0 /*總頻次*/;
...
export const initialize=function(dict,dictfreq){ //建立詞庫樹 及詞頻，並轉成對數，後面可用加法代替乘法
    const totalfreq= buildDictTrie(dict,dictfreq);
    for (let k in termfreq) {
        var v = termfreq[k];
...
var sentanceDAG = function(sentence,dict) { // 產生句子的 DAG 
...
const bestRoute = function( DAG , sentence,dict,dictfreq) { //找尋DAG最短路徑
    ...
    for (idx = N - 1; idx > -1; idx--) { //從後面開始算起，因為中文句子一般權重往後的較大
        ...
        for (let xi in DAG[idx]) { //對所有從idx出發的可能路徑
            ...
            var wfreq = dictfreq[at] || min_freq ;  //查詞頻，無此詞則視為最小機率
